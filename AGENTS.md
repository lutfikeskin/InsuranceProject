...
Agent context for Cursor. Keep this file aligned with [`CLAUDE.md`](CLAUDE.md) when you change blast-radius dependencies, architecture constraints, or feature history. Phase checklists live in [`.cursor/rules/insurance-document-platform-priority.mdc`](.cursor/rules/insurance-document-platform-priority.mdc).
...
- **Forced / fallback extraction:** Applications and quotes route through `full_policy` only when the user opts in (`allow_non_extractable=True`). Results carry `forced_extraction=True` and `source_document_type` so the UI can warn that the document was not bound coverage. Forced runs skip Gemini local cache.
...
- views/upload_queue.py — pure helpers for post-batch upload summary (KPI strip, error hints, CSV export); no Streamlit dependency
...
- **2026-05 — batch upload UX.** Branch `feat/batch-upload-ux`. Post-batch summary in `views/process_policies.py`: KPI strip, enhanced results table with retry hints, CSV exports. Logic in `views/upload_queue.py` (unit-tested in `tests/test_upload_queue.py`).
- **2026-05 — fallback extraction.** Same branch. `ExtractionPipeline.run(allow_non_extractable=True)` forces `full_policy` for applications, quotes, and user-retried failures; tags `forced_extraction` / `source_document_type`; skips cache on forced runs. UI: “Try extraction anyway” in upload step + review warning banner; bulk-save includes forced rows instead of skipping non-extractable docs silently. `document_taxonomy.py` treats application/quote as extractable via full policy when opted in.